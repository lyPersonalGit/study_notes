...
``` shell
java -jar -Dspring.profiles.active=prod -DmyParam='test1' project-name.jar
```

**方式二：命令行参数**

``` shell
java -jar project-name.jar --spring.profiles.active=prod --myParam='test2'
```

当同时以系统变量的方式与命令行参数的方式传参，如下：

``` shell
java -jar -DmyParam='test1' project-name.jar --myParam='test2'
# 此命令生效参数为 test2
```
...
``` shell
java -jar -Dserver.port=8001 -Dspring.profiles.active=prod project_name.jar
# 设置服务启动端口为8001，设置项目的配置文件为application-prod.yml
```
